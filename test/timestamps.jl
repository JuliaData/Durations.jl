...
# Test the Timestamp API. This file mirrors stdlib/Dates/test/timestamps.jl from
# JuliaLang/julia#62994; the few `TIMESTAMP_FROM_DATES` gates cover behavior the
# compatibility implementation cannot change in an older Dates stdlib.

const TIMESTAMP_FROM_DATES = Durations.TIMESTAMP_FROM_DATES
UTN(x) = Dates.UTInstant(Nanosecond(x))
const unix2timestamp = Durations.unix2timestamp
const timestamp2unix = Durations.timestamp2unix
const ISOTimestampFormat = Durations.ISOTimestampFormat
...
@testset "Gate" begin
    @test TIMESTAMP_FROM_DATES == isdefined(Dates, :Timestamp)
    if TIMESTAMP_FROM_DATES
        @test Timestamp === Dates.Timestamp
        @test ISOTimestampFormat === Dates.ISOTimestampFormat
    else
        @test Timestamp === Durations.Timestamp
        @test Dates.CONVERSION_SPECIFIERS['n'] === Nanosecond
    end
    @test Timestamp <: Dates.AbstractDateTime
    # the `n` code formats existing types too
    @test Dates.format(DateTime(2020, 1, 1, 0, 0, 0, 123), "n") == "123"
    @test Dates.format(Time(0, 0, 0, 500), "nnnnnnnnn") == "500000000"
    @test Dates.format(Time(0, 0, 0, 0, 0, 1), "n") == "000000001"
end
...
@testset "Construction and validation" begin
    ...
    @test Timestamp(UTN(0)) == Timestamp(1970)
...
@testset "Comparisons" begin
    ...
    if isdefined(Dates, :isoyear)
        @test Dates.isoyear(Timestamp(2021, 1, 1)) == Year(2020)
        @test Dates.isoweekdate(Timestamp(2021, 1, 1)) == (2020, 53, 5)
    end
...
@testset "Parsing and formatting" begin
    ...
    @test sprint(show, ts) == "Timestamp{Nanosecond}(\"2026-08-31T13:45:30.123456789\")"
    @test eval(Meta.parse(sprint(show, ts))) === ts
    ...
    # digit overflow detection in the base-10 parser is a Julia 1.14 Dates change
    TIMESTAMP_FROM_DATES && @test tryparse(Timestamp, "18446744073709553636-01-01") === nothing
...
@testset "Data layout" begin
    ...
    @test hash(ts) == hash(Timestamp(UTN(Dates.value(ts))))
    @test hash(ts) != hash(ts + Nanosecond(1))
    d = Dict(ts => 1)
    @test d[Timestamp(2026, 8, 31, 13, 45, 30, 123, 456, 789)] == 1
    # equal instants hash alike across DateTime and Timestamp; Date joins in
    # once the Dates stdlib itself defines Timestamp and the shared hash
    @test hash(Timestamp(2026, 8, 31)) == hash(DateTime(2026, 8, 31))
    @test hash(Timestamp(2026, 8, 31, 13, 45, 30, 123)) == hash(DateTime(2026, 8, 31, 13, 45, 30, 123))
    @test hash(Timestamp{Second}(2026, 8, 31)) == hash(Timestamp(2026, 8, 31))
    @test hash(Timestamp(2026, 8, 31, 0, 0, 0, 0, 1)) == hash(Timestamp{Microsecond}(2026, 8, 31, 0, 0, 0, 0, 1))
    @test hash(Timestamp(2026, 8, 31, 0, 0, 0, 0, 0, 1)) != hash(Timestamp(2026, 8, 31))
    @test hash(Timestamp{Second}(300000)) == hash(Timestamp{Millisecond}(300000))
    mixed = Dict{Any, Int}(DateTime(2026, 8, 31) => 1, DateTime(2026, 8, 31, 12) => 2)
    @test mixed[Timestamp(2026, 8, 31)] == 1
    @test mixed[Timestamp(2026, 8, 31, 12)] == 2
    @test mixed[DateTime(2026, 8, 31)] == 1
    if TIMESTAMP_FROM_DATES
        @test hash(Timestamp(2026, 8, 31)) == hash(Date(2026, 8, 31))
        @test Dict{Any, Int}(Date(2026, 8, 31) => 1)[Timestamp(2026, 8, 31)] == 1
    end
end
...
@testset "Parameterized resolution" begin
    ...
    for (P, scale) in zip(units, scales)
        ...
        for raw in (typemin(Int64), typemin(Int64) + 1, -1, 0, 1, typemax(Int64) - 1, typemax(Int64))
            ...
            (PARSES_NEGATIVE_YEARS || year(t) >= 0) && @test T(string(t)) === t
